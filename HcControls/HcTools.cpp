...
HcTools::HcTools(QObject *parent) : QObject{parent} {
}

void HcTools::clipText(const QString &text) {
    QGuiApplication::clipboard()->setText(text);
}

QString HcTools::uuid() {
    return QUuid::createUuid().toString().remove('-').remove('{').remove('}');
}

QString HcTools::readFile(const QString &fileName) {
    QString content;
    QFile file(fileName);
...
void HcTools::setQuitOnLastWindowClosed(bool val) {
    QGuiApplication::setQuitOnLastWindowClosed(val);
}

void HcTools::setOverrideCursor(Qt::CursorShape shape) {
    QGuiApplication::setOverrideCursor(QCursor(shape));
}

void HcTools::restoreOverrideCursor() {
    QGuiApplication::restoreOverrideCursor();
}

void HcTools::deleteLater(QObject *p) {
    if (p) {
        p->deleteLater();
    }
}

QString HcTools::toLocalPath(const QUrl &url) {
    return url.toLocalFile();
}

QString HcTools::getFileNameByUrl(const QUrl &url) {
    return QFileInfo(url.toLocalFile()).fileName();
}

QString HcTools::html2PlantText(const QString &html) {
    QTextDocument textDocument;
    textDocument.setHtml(html);
    return textDocument.toPlainText();
}

QRect HcTools::getVirtualGeometry() {
    return QGuiApplication::primaryScreen()->virtualGeometry();
}

QString HcTools::getApplicationDirPath() {
    return QGuiApplication::applicationDirPath();
}

QUrl HcTools::getUrlByFilePath(const QString &path) {
    return QUrl::fromLocalFile(path);
}

QColor HcTools::withOpacity(const QColor &color, qreal opacity) {
    int alpha = qRound(opacity * 255) & 0xff;
    return QColor::fromRgba((alpha << 24) | (color.rgba() & 0xffffff));
}

QString HcTools::md5(const QString &text) {
    return QCryptographicHash::hash(text.toUtf8(), QCryptographicHash::Md5).toHex();
}

QString HcTools::toBase64(const QString &text) {
    return text.toUtf8().toBase64();
}

QString HcTools::fromBase64(const QString &text) {
    return QByteArray::fromBase64(text.toUtf8());
}

bool HcTools::removeDir(const QString &dirPath) {
    QDir qDir(dirPath);
    return qDir.removeRecursively();
}

bool HcTools::removeFile(const QString &filePath) {
    QFile file(filePath);
    return file.remove();
}

QString HcTools::sha256(const QString &text) {
    return QCryptographicHash::hash(text.toUtf8(), QCryptographicHash::Sha256).toHex();
}

void HcTools::showFileInFolder(const QString &path) {
#if defined(Q_OS_WIN)
...
bool HcTools::isSoftware() {
    return QQuickWindow::sceneGraphBackend() == "software";
}

QPoint HcTools::cursorPos() {
    return QCursor::pos();
}

qint64 HcTools::currentTimestamp() {
    return QDateTime::currentMSecsSinceEpoch();
}

QIcon HcTools::windowIcon() {
    return QGuiApplication::windowIcon();
}

int HcTools::cursorScreenIndex() {
    int screenIndex = 0;
...
bool HcTools::isWindows11OrGreater() {
    static QVariant var;
    if (var.isNull()) {
#if defined(Q_OS_WIN)
...
bool HcTools::isWindows10OrGreater() {
    static QVariant var;
    if (var.isNull()) {
#if defined(Q_OS_WIN)
        auto buildNumber = windowBuildNumber();
...
QRect HcTools::desktopAvailableGeometry(QQuickWindow *window) {
    return window->screen()->availableGeometry();
}

QString HcTools::getWallpaperFilePath() {
#if defined(Q_OS_WIN)
...
QColor HcTools::imageMainColor(const QImage &image, double bright) {
    int step = 20;
    int t = 0;
    int r = 0, g = 0, b = 0;
