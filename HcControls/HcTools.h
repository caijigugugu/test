...
/**
 * @brief The HcTools class
 */
class HcTools : public QObject {
    Q_OBJECT
    QML_NAMED_ELEMENT(HcTools)
    QML_SINGLETON

private:
    explicit HcTools(QObject *parent = nullptr);

public:
    SINGLETON(HcTools)

    static HcTools *create(QQmlEngine *, QJSEngine *) {
        return getInstance();
    }
