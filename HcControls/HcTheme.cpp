...
HcTheme::HcTheme(QObject *parent) : QObject{parent} {
    _accentColor = HcColors::getInstance()->Blue();
    _darkMode = HcThemeType::DarkMode::Light;
    _nativeText = false;
    ...
    refreshColors();
    connect(this, &HcTheme::darkModeChanged, this, [=] { Q_EMIT darkChanged(); });
    connect(this, &HcTheme::darkChanged, this, [=] { refreshColors(); });
    connect(this, &HcTheme::accentColorChanged, this, [=] { refreshColors(); });
    connect(&_watcher, &QFileSystemWatcher::fileChanged, this,
            [=](const QString &path) { Q_EMIT desktopImagePathChanged(); });
    connect(this, &HcTheme::blurBehindWindowEnabledChanged, this,
            [=] { checkUpdateDesktopImage(); });
    startTimer(1000);
}

void HcTheme::refreshColors() {
    auto isDark = dark();
...
bool HcTheme::eventFilter(QObject *, QEvent *event) {
...
bool HcTheme::dark() const {
    if (_darkMode == HcThemeType::DarkMode::Dark) {
        return true;
    } else if (_darkMode == HcThemeType::DarkMode::System) {
        return _systemDark;
    } else {
        return false;
    }
...
void HcTheme::checkUpdateDesktopImage() {
    if (!_blurBehindWindowEnabled) {
        return;
    }
    QThreadPool::globalInstance()->start([=]() {
        _mutex.lock();
        auto path = HcTools::getInstance()->getWallpaperFilePath();
...
void HcTheme::timerEvent(QTimerEvent *event) {
    checkUpdateDesktopImage();
}
